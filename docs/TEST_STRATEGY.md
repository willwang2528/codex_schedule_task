...
| Task A | Scheduler 仅在交易日放行；交易日八个时点均采集；仅 `09:35`、`11:20`、`15:01` 发三张卡；仅 `15:01` 在三卡全部成功后 Pin 第一张；八时点失败均告警；`11:20`、`15:01` 有完成监控；休市日不执行也不产生缺失时点告警 | 真实任务配置行为矩阵 |
...
| 防静默失败 | 校验失败也告警；POST 已接受但回读失败保留 pending 且不重发；缺失时点写独立健康日志 | Runner 与 Scheduler 集成测试 |
