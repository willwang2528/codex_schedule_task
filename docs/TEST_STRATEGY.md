...
| 范围 | 必须保持的行为 | 主要测试层 |
| --- | --- | --- |
| Scheduler | 自动发现有效任务；跳过禁用任务和已完成时点；有待发送消息时先恢复，再执行本时点任务；宽限期后监控通知时点终态 | 临时仓库集成测试 |
| Task A | Scheduler 仅在交易日放行；交易日八个时点均采集；仅 `09:35`、`11:20`、`15:01` 发三张卡；仅 `15:01` 在三卡全部成功后 Pin 第一张；八时点失败均告警并创建一次有界自修复工单；`11:20`、`15:01` 有完成监控；休市日不执行也不产生缺失时点告警 | 真实任务配置行为矩阵 |
...
| Runner | 严格 Schema 与 Harness 双校验；机器可读纠错；Agent 不得覆盖 Harness 状态；结果无效不得污染可信状态；通知幂等；部分发送只恢复未发送消息 | 临时状态端到端测试 |
| 每日归档 | Pin 是附加能力；任何 Pin 失败都不得把原卡片发送改成失败；部分三卡发送完成前不得 Pin | 发送顺序与恢复测试 |
| 飞书适配器 | 只从环境变量取凭据；错误脱敏；成功发送和 Pin 必须返回非空 `message_id`；回读必须匹配消息 ID 与目标群 | 无网络适配器契约测试 |
| 防静默失败 | 校验失败也告警；POST 已接受但回读失败保留 pending 且不重发；缺失时点写独立健康日志；同一错误可启动一个读取同一告警的后台修复 Agent，启动失败不影响飞书告警 | Runner、修复 Worker 与 Scheduler 集成测试 |
