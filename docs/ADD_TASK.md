...
For tasks that must never fail silently, add `delivery.failure_alert` with `enabled: true` and a non-empty `trigger_slots` subset of `schedule.triggers`. Failure alerts are deterministic Post messages generated by the Harness, not by the Agent. They may cover data-only slots and use the task's configured Feishu chat route. A failed alert remains pending for normal recovery.

To start one bounded local repair Agent from the same sanitized incident, add `failure_alert.self_repair` with `enabled: true` and a positive `timeout_seconds`. The repair request is written under `logs/self-repair/<task-id>/` before a background `workspace-write` Agent starts. It reads the same alert plus local output paths, follows repository TDD rules, and records its status and final response there. This optional path never blocks the original Feishu alert and never commits, pushes, sends messages, or reruns a business task.

To verify real delivery, add `delivery.readback`. Its `trigger_slots` must be notification-enabled; `include_failure_alerts: true` applies the same verification to incident messages. To detect a missing run before pending exists, add `delivery.completion_monitor`; its slots must be notification-enabled and present in `failure_alert.trigger_slots`.

Declare every Agent-owned state namespace in `state.allowed_update_keys`. Each name must also exist in the Schema namespace enum. Agent results use `state_updates` as an array of `{namespace, operation, value_json}` records, with `operation` fixed to `upsert`; Harness-owned fields are never valid namespaces. Invalid JSON values, duplicate namespaces, and task-disallowed namespaces are rejected before state is written.
...
- Every state namespace is declared in both `task.yaml` and the result Schema.
